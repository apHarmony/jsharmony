...
exports = module.exports = function(jsh){
  ...
  XExt.CustomPrompt = function (sel, html, onInit, onAccept, onCancel, onClosed, options) {
    ...
    if(options.backgroundClose){
      var mouseDownEventName = 'mousedown.jsharmonyCustomPromptBgClose' + sel;
      var mouseUpEventName = 'mouseup.jsharmonyCustomPromptBgClose'  + sel;
      var xdialogblock = jsh.$root('.xdialogblock');
      $(window).off(mouseDownEventName).on(mouseDownEventName, function(e1){
        xdialogblock.off(mouseUpEventName);
        if(!jsh.xDialog.length) $(window).off(mouseDownEventName);
        if(e1.target !== xdialogblock[0]) return;
        xdialogblock.one(mouseUpEventName, function(e2){
