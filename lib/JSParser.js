...
exports.ParseTree = function (formatter, tokens, filename, options) {
  ...
  //OBJECT
  var cur_item = null, last_item = null, ptoken = null, aval = null;
  ...
  if (token.Name == 'LCBRACKET') {
    ...
    do {
      ...
      if (token.Name == 'STRING') {
        ...
        if (last_item && (last_item == 'COMMA')) { /* Do nothing */ }
    ...
    do {
      ...
      if (ptoken.Name == 'RSBRACKET') {
        ...
        if ((PeekToken().Name == 'LCBRACKET') && (Peek2Token().Name == 'RCBRACKET')) { vmid = Peek2Token().Pre; }
        if ((PeekToken().Name == 'LSBRACKET') && (Peek2Token().Name == 'RSBRACKET')) { vmid = Peek2Token().Pre; }
        aval = exports.ParseTree(vformatter, tokens, filename, options);
        if (tokens.length > 0) vpost = PeekToken().Pre;
        rslt.push(aval);
    ...
    if (isNaN(rslt)) throw new ParseError(debuginfo, token);
  }
...
exports.GenString = function(val, formatter, isChild, parentf, options){
  var rslt = '';
  var first = true, thisf = null, vformatter = [];
  ...
  if (formatter.length && formatter[0].VSource) rslt = formatter[0].VSource;
    ...
    for (var key in val) {
      ...
      for(var k=0;k<formatter.length;k++){
        if (formatter[k].S == key) { thisf = formatter[k]; }
      }
      vformatter = [];
...
exports.ParseJSON = function(str, desc, options){
  var rslt  = null;
  options = _.extend({ trimErrors: false }, options);
  try {
    rslt = exports.Parse(str, desc, options).Tree;
  }
  catch (ex) {
    if('startpos' in ex){
      var errmsg = (desc ? desc + ' - invalid' : 'Invalid') + ' JSON on line ' + ex.startpos.line + ', char ' + ex.startpos.char + '\n';
      var eline = getLine(str, ex.startpos.line);
      if(typeof eline != 'undefined'){
        if(options.trimErrors){
          while(eline.length && ((' \t\n\r\v\f').indexOf(eline[0]) >= 0)){
            eline = eline.substr(1);
            ex.startpos.char--;
          }
        }
        errmsg += eline + '\n';
        for(var i=0;i<ex.startpos.char;i++) errmsg += '-';
        errmsg += '^\n';
      }
      errmsg += ex.message + '\n';
      var ex2 = new Error(errmsg);
      ex2.stack = ex.stack;
      throw ex2;
    }
    else throw ex;
