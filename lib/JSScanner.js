...
exports.Scan = function(str, filename) {
  ...
  var input = { Pos: 0, Line: 1, File: filename, StartPos: 0, StartLine: 1, StartPre: '', FilePos: 0, StartFilePos: 0 };
  var strpos = 0;
  ...
  while (strpos < strlen) {
    ...
    if (IsDigit(c) || (c == '-')) {
      ...
      tokens.push(new JSToken.NUMBER(input, val, isScientific));
      //Backtrack
    ...
    if (c == '@') {
      ...
      while ((strpos < strlen) && (c != '(') && !IsWhiteSpace(c) && (c !== ''));
      //Add token
      tokens.push(new JSToken.FUNCTION(input, val));
      //Backtrack
      if(strpos != strlen) strpos--;
      continue;
    }
    ...
    else if (c == '(') tokens.push(new JSToken.LPAREN(input));
    else if (c == ')') tokens.push(new JSToken.RPAREN(input));
  ...
  tokens.push(new JSToken.END(input, ws));
  return tokens;
};
