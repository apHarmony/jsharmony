      ...
      db.meta.getTableFields(tabledef, function(err, messages, fields){
        ...
        async.waterfall([
          ...
          function(generate_cb){
            _this.generateModelFromTableDefition(tabledef, 'form', { db: options.db }, function(err, messages, modeltxt){  
          ...
          //Generate Grid
          function(generate_cb){
            _this.generateModelFromTableDefition(tabledef, 'grid', { db: options.db, form: model_name+'_form' }, function(err, messages, modeltxt){  
              if(err) return generate_cb(err);
              if(messages.length > 0) rsltmessages = rsltmessages.concat(messages);
...
CodeGen.prototype.generateModelFromTableDefition = function(tabledef, layout, options, callback){
  var _this = this;
  options = _.extend({ db: 'default' /* ,form: model_id */ }, options);
  if(!options.db) options.db = 'default';
  ...
  _.each(tabledef.fields,function(fielddef){
    ...
    if(layout=='form'){
      ...
      else if(finaltypename=='binary'){ field.control = 'textbox_L'; /* xxx */ }
