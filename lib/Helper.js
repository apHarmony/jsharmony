...
exports.GetModelRoles = function(req, model){
  ...
  if((req.jshconfig.id in roles) && !_.isString(roles[req.jshconfig.id])) roles = roles[req.jshconfig.id];
  else if(req.jshconfig.id != 'main') return {};
  return roles;
}
...
exports.SendEmail = function (mparams,callback){
  if(global.debug_params.disable_email){ global.log.info('DEBUG - NO EMAIL SENT'); return callback(); }
  if (!('from' in mparams)) mparams.from = global.mailer_email;
  global.log.info(mparams);
  if(!global.mailer){ global.log.error('ERROR - global.mailer not configured'); return callback(); }
...
exports.ParamCheck = function (desc, col, params, show_errors) {
  ...
  if (typeof params != 'undefined') {
    for (var i = 0; i < params.length; i++) {
      ...
      var req = (params[i][0] == '&');
      if (req && !(param in col)) {
        if (show_errors) global.log.warning(desc + ': Invalid Parameters - Missing ' + param);
        return false;
      }
      parsed.push(param);
    }
  ...
  for (var i in col) {
    if (!_.includes(parsed, i)) {
      if (show_errors) global.log.warning(desc + ': Invalid Parameters - Extra Parameter ' + i);
      return false;
    }
  }
  return true;
...
exports.GetCookie = function(req, res, jsh, name){
  name += exports.GetCookieSuffix(req);
...
exports.GetCookieSuffix = function(req){
  if(typeof global.jshSettings.cookie_suffix !== 'undefined') return global.jshSettings.cookie_suffix;
