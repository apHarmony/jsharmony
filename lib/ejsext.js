...
exports.getActions = function (req, model){
  ...
  var effperm = '';
  if (('DEV' in req._roles) || (!model.dev && ('SYSADMIN' in req._roles))){
    //Get union of all roles
    let roles = Helper.GetModelRoles(req, model);
    if(_.isEmpty(roles)) effperm = '*';
    else {
...
exports.BreadCrumbs = function (req, jsh, fullmodelid) {
  if (req.bcrumb_override) { return '<div class="bcrumbs">'+req.bcrumb_override+'</div>'; }
  var model = jsh.getModel(req, fullmodelid);
  if (!model) return;
  if (!('breadcrumbs' in model)) return;
  ...
  if((typeof breadcrumbs_title == 'undefined') && (typeof breadcrumbs_parents == 'undefined')) return;
...
exports.null_blank = function(val){
...
exports.getLOV = function (jsh, LOV, val) {
...
exports.getLOVTxt = function (jsh, LOV, val) {
  var lov = exports.getLOV(jsh, LOV, val);
  if(lov) return lov[jsh.uimap.code_txt];
  return undefined;
};
exports.getFieldByName = function (model, fieldname){
