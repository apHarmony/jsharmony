...
//access (req, model, perm1, perm2, perm3, permN )
//Run getaccess on [req, model, perm1, ... permN-1]
...
//getaccess(req, model, perm1, perm2, perm3, ....)
...
exports.BreadCrumbs = function (req, jsh, fullmodelid) {
  ...
  var targetperm = '';
  if(req && req.query && req.query.action=='add') targetperm = 'I';
  else targetperm  = 'B';

  var breadcrumbs_parents = undefined;
  var breadcrumbs_title = undefined;

  if((targetperm == 'I') && breadcrumbs.add){
    if('parents' in breadcrumbs.add) breadcrumbs_parents = breadcrumbs.add.parents;
    if('title' in breadcrumbs.add) breadcrumbs_title = breadcrumbs.add.title;
  }
  else if((targetperm == 'B') && breadcrumbs.edit){
    if('parents' in breadcrumbs.edit) breadcrumbs_parents = breadcrumbs.edit.parents;
    if('title' in breadcrumbs.edit) breadcrumbs_title = breadcrumbs.edit.title;
  }
...
exports.getHelpURL = function (req, jsh, model){
  var help_view = exports.getHelpView(req, jsh);
  var help_view_model = jsh.getModel(req, help_view);
  if(!help_view_model) return '';
  var helpid = '';
  if(model.helpid) helpid = model.helpid;
  return req.baseurl + help_view_model.id + '/?' + jsh.Config.help_panelid + '=' + encodeURIComponent(helpid); //help_listing
}
exports.getHelpOnClick = function (req, jsh, model) {
  var help_view = exports.getHelpView(req, jsh);
...
exports.getHelpView = function(req, jsh){
  ...
  for(var siteid in jsh.Config.help_view){
    if(req.jshsite.id==siteid) return jsh.Config.help_view[siteid];
  }
  return '';
