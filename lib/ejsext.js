...
exports.getHelpView = function(req, jsh){
  ...
  for(var siteid in jsh.Config.help_view){
    if(req.jshconfig.id==siteid) return jsh.Config.help_view[siteid];
  }
...
exports.null_log = function (val, txt){
  if (typeof val === undefined) global.log.warning('Report has undefined value: ' + txt);
  if (val == null) global.log.warning('Report has null value: ' + txt);
  return (val||'');
}
