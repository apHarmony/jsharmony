...
exports.getaccess = function (req, model){
  ...
  if (!('access' in model)) return '';
  var effperm = '';
  if ('SYSADMIN' in req._roles) effperm = '*';
  ...
  effperm = this.intersectperm(effperm, model.access);
...
exports.accessField = function (req, field, perm) {
  ...
  if (perm == "*") return true;
  if (field.access === undefined) return false;
...
exports.BreadCrumbs = function (req, jsh, modelid) {
  ...
  _.each(breadcrumbs.parents, function (bcrumb) {
    if (bcrumb in jsh.Models) {
      var blink = req.baseurl + bcrumb;
      var mtitle = jsh.Models[bcrumb].title;
      if (('breadcrumbs' in jsh.Models[bcrumb]) && ('title' in jsh.Models[bcrumb].breadcrumbs)) mtitle = jsh.Models[bcrumb].breadcrumbs.title;
...
exports.getHelpURL = function (req, jsh, modelid){
  var helpid = modelid;
  if (modelid in jsh.Models) helpid = jsh.Models[modelid].helpid;
  return req.baseurl + jsh.Config.help_view + '/?' + jsh.Config.help_panelid + '=' + encodeURIComponent(modelid); //help_listing
}
exports.getHelpOnClick = function (req, jsh) {
  if(!jsh.Config.help_view) return 'XExt.Alert(\'Help not initialized.\'); return false;';
  return jsh.getModelLinkOnClick(jsh.Config.help_view);
}
