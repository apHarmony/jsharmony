...
exports.fileNotFound = function(err){
...
exports.tryUnlink = function (path, callback) {
  fs.unlink(path, function (err) {
    if(exports.fileNotFound(err)) return callback(null);
    if(err) return callback(err);
    return callback(null);
  });
};
...
exports.getMimeType = function(fname) {
  fname = path.basename(fname);
  return mime.lookup(fname);
};
exports.copyFile = function(source, target, cb, options) {
  ...
  rd.on('error', done);
  if(options.onData) rd.on('data', options.onData);
...
exports.cleanFileName = function (fname, replaceChar){
  ...
  return fname.replace(/[/?<>\\:*|":]/g, replaceChar).replace(/[\x00-\x1f\x80-\x9f]/g, replaceChar).replace(/^\.+$/, replaceChar).replace(/^(con|prn|aux|nul|com[0-9]|lpt[0-9])(\..*)?$/i, replaceChar); //eslint-disable-line no-control-regex
};
...
exports.rmdirRecursive = function (fpath, cb){
  ...
  }, { file_before_dir: true }, cb);
};
exports.copyRecursive = function (source, target, options, cb){
  options = _.extend({
    forEachFile: function(filepath, targetpath, relativepath, cb){ return cb(true); },
    forEachDir: function(dirpath, targetpath, relativepath, cb){ return cb(true); }
  }, options);
  return exports.funcRecursive(source, function (filepath, relativepath, file_cb) { //filefunc
    ...
    options.forEachFile(filepath, targetpath, relativepath, function(copy){
      if(!copy) return file_cb();
      exports.copyFile(filepath, path.join(target, relativepath), file_cb);
    });
  }, function (dirpath, relativepath, dir_cb) { //dirfunc
    var targetpath = path.join(target, relativepath);
    options.forEachDir(dirpath, targetpath, relativepath, function(create){
...
exports.convertWindowsToPosix = function (path){
  if(!path) return '';
  if(Helper.beginsWith(path, '\\\\?\\')) path = path.substring(4);
  path = path.toString().split('\\').join('/');
  return path;
};
