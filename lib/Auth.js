...
var Auth = function (req, res, onSuccess, onFail) {
  ...
  if (req.jshsite.auth.preprocess_account) { req.jshsite.auth.preprocess_account(this.AppSrv, account, _Xauth, onFail); }
  else _Xauth();
}
  ...
  req.jshsite.auth.on_auth(req, jsh, sqlparams, function (err, rslt) {
    ...
    if ((rslt != null) && (rslt.length == 1) && (rslt[0].length == 2) && (rslt[0][0].length == 1)) {
      ...
      if (user_info[jsh.map.user_status].toUpperCase() != 'ACTIVE'){
        if(jsh.Config.debug_params.auth_debug) jsh.Log('Auth: User status not ACTIVE'); 
        return onFail();
      }
      else {
        ...
        if(jsh.Config.debug_params.auth_debug){
          jsh.Log('Auth DB Hash:     '+posthash);
          jsh.Log('Auth Client Hash: '+account.password);
        }
        if (posthash == account.password) {
          ...
          if(jsh.Config.debug_params.auth_debug) jsh.Log('Auth: Success');
      ...
      if(jsh.Config.debug_params.auth_debug) jsh.Log('Auth: User not found'); 
      return onFail();
    }
  });
};
