...
var Auth = function (req, res, onSuccess, onFail) {
  var account =  Auth.GetAccountCookie(req, this);
  var _this = this;
...
Auth.SetAccountCookie = function(req, res, jsh, account){
  const expiration = account.remember ? new Date(Date.now() + 31536000000) : false;
  signAccountCookie(account, req.jshsite.auth.salt);
  Helper.SetCookie(req, res, jsh, 'account', account, { 'expires': expiration, 'path': req.baseurl });
};
Auth.ClearAccountCookie = function(req, res, jsh){
  Helper.ClearCookie(req, res, jsh, 'account');
}
Auth.GetAccountCookie = function(req, jsh){
  const cookie = Helper.GetCookie(req, jsh, 'account') || {};
  return verifyAccountCookieSignature(cookie, req.jshsite.auth.salt) ? cookie : {};
};
Auth.Logout = function(req, res, jsh) {
  const account = {
    username: '',
    password: '',
    remember: false,
  ...
  const accountCookie = Auth.GetAccountCookie(req, jsh);
  if (accountCookie) {
    ...
    if ('password' in accountCookie && 'username' in accountCookie) req.jshsite.clearRefreshTokenStore(accountCookie.username, accountCookie.password);    
  }
  Auth.SetAccountCookie(req, res, jsh, {
    username: '',
    password: '',
    remember: false,
    tstmp: ''
    ...
    if ((rslt != null) && (rslt.length == 1) && (rslt[0].length == 2) && (rslt[0][0].length >= 1)) {
      ...
      if ((user_info[jsh.map.user_status]||'').toUpperCase() != 'ACTIVE'){
        ...
        req.jshsite.auth.getTrustedToken(req, jsh, user_info, function(error, token){
          ...
          const isExpired = Date.now() > account.expires;
          if (isExpired) {
...
function signAccountCookie(account, salt) {
  account.__ = '';
  const hmac = crypto.createHmac('sha3-512', salt);
  hmac.update(JSON.stringify(account));
  account.__ = hmac.digest('base64');
}

function verifyAccountCookieSignature(account, salt) {
  const originalSignature = account.__;
  signAccountCookie(account, salt);
  const signaturesMatch = account.__ === originalSignature;
  account.__ = originalSignature;
