...
Logger.prototype.log = function(txt, options) {
  ...
  _this.events.emit('any', logObject);
  if(logtype=='ERROR') _this.events.emit('error', logObject);
  else if(logtype=='WARNING') _this.events.emit('warning', logObject);
  else if(!logtype) _this.events.emit('info', logObject);
};
