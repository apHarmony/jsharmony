...
function Logger(platform){
  ...
  rslt.debug = function(txt, options){ console.log(txt); if(_this.platform.Config.debug_params.debug_log_to_disk){ _this.log(txt, _.extend({ logtype: 'DEBUG' }, options)); }}; // eslint-disable-line no-console
  ...
  rslt.express.with = function(options){ return function(req, res, next){ _this.express(req, res, next, options); }; };
  ...
  rslt.getQueue = function(){ return _this.logqueue; };
  rslt.flush = function(){ _this.logProcessor(true); };
...
Logger.prototype.log = function(txt, options) {
  ...
  var color = options.color;     //Color when writing to console
  var ext = options.ext;         //Log file extension (YYYYMMDD.log.EXT)
  var logtype = options.logtype; //ERROR, WARNING, <blank>
  var source = options.source;   //system, webserver, ...
  var logfile = options.logfile; //Log directly to alternate file. (color, ext, logtype, source parameters are not applicable)
  ...
  if(logfile){
    this.logqueue.push({ txt: logstr, logfile: logfile });
  }
  else {
    if (!ext) ext = '';
    ...
    if (!_this.platform.Config.debug_params || _this.platform.Config.debug_params.pipe_log) {
      if(color) console.log('\033[' + _this.getConsoleColor(color) + 'm' + logstr + '\033[0m'); // eslint-disable-line no-console
      else console.log(logstr); // eslint-disable-line no-console
...
Logger.prototype.logProcessor = function(force) {
  ...
  while (_this.logqueue.length > 0) {
    let logdata = _this.logqueue.shift();
    ...
    //Get log file path
    var logfile = '';
    if(logdata.logfile) logfile = logdata.logfile;
    else{
      logfile = _this.platform.Config.logdir + moment().format('YYYYMMDD') + '.log';
      if (logdata.ext) logfile += '.' + logdata.ext;
    }
    //Write to log file
    if (force) {
      if(logtype && (logtype.toUpperCase()=='ERROR')){
  ...
  if(_this.platform.Mailer && _this.platform.Config.error_email && !_this.isSendingEmail){
    if(_this.lastError){
      var errmsg = _this.lastError.substr(0,1000);
    ...
    while(_this.logmailqueue.length > 0){
      let logdata = _this.logmailqueue.shift();
      _this.sendErrorEmail(logdata.txt, function(){ _this.logProcessor(); });
      return;
    }
  }
  if(!_this.platform.Config.interactive && !force){
    setTimeout(function () { _this.logProcessor(); }, _this.platform.Config.LogSleepDelay);
  }
};

//Express Web Server logging
Logger.prototype.express = function(req, res, next, options) {
  var _this = this;
  req._startTime = new Date;
  var log_on_end = true;
  options = _.extend({ pathPrefix: '' }, options);
  function log_express() {
    ...
    var txt = '' +
      req.method + ' ' +
      options.pathPrefix + (req.originalUrl || req.url) + ' ' +
