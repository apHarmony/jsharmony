...
exports = module.exports = function (req, res, onComplete){
  var jsh = this;
  if (!(req.secure) && !(req.jshconfig.auth.allow_insecure_http)) { return Helper.GenError(req, res, -21, 'Secure connection required'); return; }
      ...
      if ((rslt != null) && (rslt.length == 1) && (rslt[0].length == 2) && (rslt[0][0].length >= 1)) {
        ...
        for (var i = 0; i < rslt[0][0].length; i++) {
          ...
          if (user_info[jsh.map.user_status].toUpperCase() == 'ACTIVE') {
            ...
            var reset_link = Helper.getFullURL(req, req.baseurl + 'login/forgot_password_reset?email=' + encodeURIComponent(email) + '&key=' + crypto.createHash('sha1').update(PE_ID + req.jshconfig.auth.salt + PE_LL_Tstmp).digest('hex'));
